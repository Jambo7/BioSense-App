...
export default async function ReportsPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto fade-up space-y-8">
      {/* Page header */}
      <header className="flex items-start justify-between gap-4">
        <div>
          <div className="text-eyebrow uppercase text-sage-deep mb-2">Intelligence reports</div>
      ...
      <Card padding="md" className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-body-sm font-semibold text-ink">Notify me when reports drop</div>
      ...
      {weeklyReports.length > 0 && (
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <Pill tone="ink" size="sm" className="ml-1">{weeklyReports.length}</Pill>
          </div>
          <div className="space-y-2.5">
            {weeklyReports.map((r) => {
              const content = (r.content as Record<string, unknown>) ?? {}
              const headline = typeof content.headline === 'string' ? content.headline : 'Weekly summary'
              return (
                <Card key={r.id} className="cursor-pointer transition-all hover:border-line-2 hover:bg-off-white group">
                  <div className="flex items-center gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5">
                        <CardLabel className="mb-0">Week {r.period}</CardLabel>
                        <Pill tone="soft-sage" size="sm">{r.checkinsCompleted ?? 0}/7 check-ins</Pill>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-x-3 gap-y-1 text-caption text-ink-3 mt-1.5">
                        {r.bestDay && <span>Best: <span className="text-sage-deep font-medium">{r.bestDay}</span></span>}
                        {r.worstDay && <span>Worst: <span className="text-rose font-medium">{r.worstDay}</span></span>}
                      </div>
                    </div>
      ...
      {monthlyReports.length > 0 && (
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <Pill tone="ink" size="sm" className="ml-1">{monthlyReports.length}</Pill>
          </div>
          <div className="space-y-2.5">
            {monthlyReports.map((r) => {
              const content = (r.content as Record<string, unknown>) ?? {}
              const summary = typeof content.summary === 'string' ? content.summary : 'Monthly summary'
              return (
                <Card key={r.id} className="cursor-pointer transition-all hover:border-line-2 hover:bg-off-white group">
                  <div className="flex items-center gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-body-sm text-ink-2 leading-relaxed line-clamp-2">{summary}</p>
                    </div>
                    <ChevronRight className="w-5 h-5 text-ink-3 group-hover:text-ink shrink-0" />
                  </div>
                </Card>
        ...
        <div className="grid sm:grid-cols-2 gap-4 mt-2">
          <div className="flex items-start gap-3">
            ...
            <div className="text-caption text-ink-2 leading-relaxed">
              ...
              Sunday 7am · headline, what changed, why it happened, 3 actions, effort vs impact, best/worst day.
            </div>
          </div>
          <div className="flex items-start gap-3">
            <IconBadge icon={CalendarClock} tone="amber" size="sm" />
            <div className="text-caption text-ink-2 leading-relaxed">
              ...
              Last day of month · progress graphs, patterns, personal drivers, biological age trend.
            </div>
          </div>
        </div>
      </Card>
 ...
 * Mirrors the moodboard weekly report layout (callout, what-changed +
 * why-it-happened, 3 actions + effort/impact, pattern snapshot,
 * consistency, final note).
 */
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-4">
        ...
        <Pill tone="ink" size="sm">Preview · not your data</Pill>
      </div>
      ...
      <Card padding="lg" className="space-y-6">
        ...
        <div className="rounded-[12px] bg-rose-tint border border-[rgba(201,122,122,0.25)] p-5 flex items-center gap-5">
          ...
          <div className="flex-1">
            ...
            <p className="text-body text-ink leading-snug">
              Your energy dropped this week — mainly driven by <span className="font-semibold">inconsistent sleep</span>{' '}
              and <span className="font-semibold">rising stress mid-week</span>.
            </p>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <Card variant="soft" padding="md">
            ...
            <div className="space-y-2">
              {[
                { label: 'Energy', delta: '↓ 21%', tone: 'rose' as const },
                { label: 'Sleep consistency', delta: '↓ 26%', tone: 'rose' as const },
                { label: 'Stress', delta: '↑ 18%', tone: 'rose' as const },
                { label: 'Resting heart rate', delta: '↑ 3 bpm', tone: 'rose' as const },
              ].map((r) => (
          ...
          <Card variant="soft" padding="md">
            ...
            <p className="text-body-sm text-ink-2 leading-[1.65]">
              Your sleep window shifted later in the week (Tue–Thu), with bedtimes moving 60–90 minutes later than usual. This
              compounded mid-week stress and likely drove the dip in recovery.
            </p>
          </Card>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <Card variant="sage" padding="md">
            ...
            <p className="text-body-sm text-ink leading-[1.65]">
              Your energy tends to drop <strong>2 days after</strong> disrupted sleep — not immediately. The impact
              of poor sleep often shows up later, not the next day.
            </p>
          </Card>
          ...
          <Card variant="soft" padding="md">
            ...
            <div className="text-caption text-ink-2 mb-3">
              Best: <span className="text-sage-deep font-semibold">Monday</span>{' '}
              · Worst: <span className="text-rose font-semibold">Thursday</span>
            </div>
        ...
        <div className="rounded-[12px] border border-line p-4 flex items-center gap-3 bg-off-white">
          ...
          <div className="flex-1">
            <div className="text-body-sm font-semibold text-ink">Consistency check · 5/7 days</div>
            <div className="text-caption text-ink-2">You&apos;re building a strong data baseline — keep going.</div>
          </div>
        ...
        <div className="rounded-[12px] bg-sand-deep/40 border border-line p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-caption text-ink-2 leading-relaxed">
              This wasn&apos;t your strongest week, but the pattern is clear — and very fixable. Small improvements
              in sleep timing alone could reverse most of this in the next 7 days.
            </p>
          </div>
        </div>
      </Card>
